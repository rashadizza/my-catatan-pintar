...
<body>
    <!--====== SIDE PART START ======-->
    <div class="flex-container min-h-screen bg-white" style="background-image: url('{{ asset('bg_pass.png') }}');"
                ...
                <div class="overflow-x-auto relative shadow-md sm:rounded-lg">
                    <div class="p-4 flex justify-between items-center bg-blue-100">
                        ...
                        <a href="/passwords/create"
                            class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded transition ease-in-out duration-300">Tambah
                            Akun</a>
                    </div>
                    <table class="w-full text-base text-left text-black">
                        ...
                        <tbody>
                            @foreach ($passwords as $index => $password)
                                <tr
                                    class="bg-blue-50 border-b dark:bg-blue-200 dark:border-blue-800 hover:bg-blue-100 dark:hover:bg-blue-400">
                                    ...
                                    <td class="py-4 px-5">
                                        <input type="password" value="{{ decrypt($password->password_encrypted) }}"
                                            class="password-field" readonly>
                                        <button onclick="togglePasswordVisibility(this)"
                                            class="toggle-password bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-2 rounded">Show</button>
                                    </td>
                                    <td class="py-4 px-5 flex space-x-2">
                                        <a href="{{ route('passwords.edit', $password->id) }}"
                                            class="bg-blue-500 hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 text-white inline-flex items-center transition ease-in-out duration-300">
                                            Edit
                                        </a>
                                        <form action="{{ route('passwords.destroy', $password->id) }}" method="POST"
                                            ...
                                            @method('DELETE')
                                            <button type="submit"
                                                class="bg-red-500 hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-4 py-2 text-white inline-flex items-center transition ease-in-out duration-300">
                                                Delete
                                            </button>
                                        </form>
                                    </td>
